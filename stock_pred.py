...
def main():
    ...
    stock_data = pd.read_csv(r'C:\Users\hemua\Desktop\stock_data.csv', parse_dates=['Date'], index_col='Date')
    X = stock_data['Close']
    ...
    if input_date <= next_day and input_date >= start_day + datetime.timedelta(days=20):
        ...
        st.error(
            'Error: Either the date is above the last date of the dataset OR below the start date + 20 days of the dataset. Please enter a date between or equal to {} and {} !!'.format(
                start_day + datetime.timedelta(days=20), next_day))
